...
class SpatialPlanningAssessment {
    ...
    async downloadPdfResults() {
        ...
        downloadButton.textContent = 'Генеруємо звіт...';
        ...
        try {
            ...
            pdf.setFontSize(18);
            pdf.text('Звіт спроможності громади', pdfWidth / 2, yPos, { align: 'center' });
            yPos += 30;

            pdf.setFont('Arial', 'normal');
            pdf.setFontSize(12);
            pdf.text(`Назва громади: ${this.hromadaInfo.hromada}`, margin, yPos);
            yPos += 20;
            pdf.text(`Область: ${this.hromadaInfo.oblast}`, margin, yPos);
            yPos += 20;
            pdf.text(`Роль: ${this.hromadaInfo.roleName}`, margin, yPos);
            yPos += 20;
            pdf.text(`Дата: ${new Date().toLocaleDateString('uk-UA')}`, margin, yPos);
            yPos += 30;
            ...
            pdf.addImage(chartImage, 'PNG', chartX, yPos, chartWidth, chartHeight);
            yPos += chartHeight + 30;

            // --- Add Scores Table ---
            const scoresHtml = this.createScoresHtmlForPdf(this.results.scores, true);
            const scoresElement = document.createElement('div');
            scoresElement.innerHTML = scoresHtml;
            document.body.appendChild(scoresElement); // Must be in DOM for html2canvas
            
            const scoresCanvas = await html2canvas(scoresElement, { scale: 2 });
            const scoresImage = scoresCanvas.toDataURL('image/png', 1.0);
            const scoresAspectRatio = scoresCanvas.width / scoresCanvas.height;
            const scoresWidth = contentWidth;
            const scoresHeight = scoresWidth / scoresAspectRatio;
            
            if (yPos + scoresHeight > pdf.internal.pageSize.getHeight() - margin) {
                pdf.addPage();
                yPos = margin;
            }
            pdf.addImage(scoresImage, 'PNG', margin, yPos, scoresWidth, scoresHeight);
            yPos += scoresHeight + 20;
            
            document.body.removeChild(scoresElement); // Clean up
            ...
            pdf.text('Рекомендації', margin, yPos);
            yPos += 25;
            ...
            for (const rec of this.recommendations) {
                const sphereTitle = `— ${window.SPHERES[rec.sphereId].name}`;
                const lines = pdf.splitTextToSize(rec.text, contentWidth - 10);
                
                const requiredHeight = (lines.length + 2) * 12; 
                ...
                pdf.setFont('Arial', 'bold');
                pdf.text(sphereTitle, margin, yPos);
                yPos += 15;
            ...
            const fileName = `Spatial_Planning_Assessment_${this.hromadaInfo.hromada}.pdf`;
            ...
            alert("Виникла помилка при створенні PDF. Будь ласка, спробуйте ще раз.");
        } finally {
            downloadButton.textContent = 'Завантажити результати в PDF';
